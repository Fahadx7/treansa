...
async function handleMultiAgent(request, env) {
  ...
  if (!apiKey) return json({ success: false, error: 'مفتاح Gemini غير مضبوط في الخادم' }, 503);
...
ATR(14): ${atr ?? 'N/A'} | موجة إليوت: ${wave ?? 'غير محدد'} | نسبة الحجم: ${volumeRatio ?? 'N/A'}x`;
    ...
    const [technical, fundamental, sentiment, risk] = await Promise.all([
      callGemini(apiKey, `أنت محلل فني خبير في السوق السعودي. حلل البيانات التالية وقدم تحليلاً فنياً مختصراً (3-5 نقاط):
${stockCtx}
ركز على: اتجاه RSI/MACD، مستويات الدعم والمقاومة، موجة إليوت، إشارات الدخول/الخروج.`),

      callGemini(apiKey, `أنت محلل أساسي خبير في السوق السعودي. قدم تحليلاً أساسياً مختصراً (3-5 نقاط) لـ:
${stockCtx}
ركز على: القطاع، النمو المتوقع، العوامل الاقتصادية المؤثرة، مقارنة بالقطاع.`),

      callGemini(apiKey, `أنت محلل معنويات السوق السعودي. حلل المعنويات المحيطة بـ:
${stockCtx}
ركز على: اتجاه السوق العام، حجم التداول مقارنة بالمتوسط، ضغط البيع/الشراء، ثقة المستثمرين.`),

      callGemini(apiKey, `أنت خبير إدارة مخاطر في السوق السعودي. قيّم المخاطر لـ:
${stockCtx}
ركز على: نسبة المخاطرة/العائد، وقف الخسارة المقترح (بناءً على ATR)، حجم المركز المناسب، مخاطر القطاع.`),
    ]);

    // Synthesis
    const synthesis = await callGemini(apiKey, `أنت كبير المحللين في السوق السعودي. لديك تقارير 4 محللين:
